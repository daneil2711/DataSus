...
class IngestaoBronze:
    ...
    def default_query(self):
        ids = ",".join(self.id_fields)
        window = f"""row_number() OVER (PARTITION BY {ids} ORDER BY {self.timestamp_field} DESC) as filter"""
        base_query = f"""
        SELECT * FROM (
        SELECT *,
            NOW() as ingestion_at,
            ...
            """
        final_query = """
        FROM {table} ) tmp where filter = 1"""
        
        
        return base_query + final_query
    #CloudFiles é só do Bricks
